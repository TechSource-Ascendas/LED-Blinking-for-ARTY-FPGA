...
| LEDs General Purpose (4 LED)  | OUT | 4     | H5, J5, T9, T10                 | LD4..LD7 only          |
...
**Do not use the MathWorks-shipped board** "Xilinx Artix-7 35T Arty
development board" (`matlabroot\toolbox\hdlcoder\boards\amd\+XilinxArty`) for
LED demos on these same 4 pins — its built-in `LEDs General Purpose` interface
hardcodes `IOSTANDARD = LVCMOS15` for `H5,J5,T9,T10`, which conflicts with
`clk` (`LVCMOS33`) in the same I/O bank (35) and fails `place_design` with
`ERROR: [DRC BIVC-1] Bank IO standard Vcc`. This custom board's interfaces are
the corrected replacement.

## Usage (GUI)

1. `addpath('<path-to-this-repo>\MathWorks\arty_custom_board')`
...
5. Task 1.3 **Set Target Interface**: map `led` to
   `LEDs General Purpose (4 LED) [0:3]` (or `LEDs General Purpose [0:7]` if
   the model's `led` port is 8 bits wide instead of 4).
...
```matlab
addpath('<path-to-this-repo>\MathWorks\arty_custom_board');
hdlsetuptoolpath('ToolName','Xilinx Vivado', ...
    'ToolPath','C:\Xilinx\Vivado\2024.1\bin\vivado.bat');  % adjust to your Vivado install
...
hdlset_param('flash_led/flash_led_top/led', ...
    'IOInterface','LEDs General Purpose (4 LED) [0:3]');  % for the 4-LED port variant
